...
export async function create(req: Request, res: Response): Promise<void> {
  ...
  const { rows } = await pool.query(
    ...
    [
      category_id ?? null, slug, title_en, title_ar,
      subtitle_en ?? null, subtitle_ar ?? null,
      image_url, swatch_color ?? null, sort_order ?? 0, is_active ?? true,
      description_en ?? null, description_ar ?? null,
      best_use_en ?? null, best_use_ar ?? null,
      eco_friendly_en ?? null, eco_friendly_ar ?? null, model_3d ?? null,
      JSON.stringify(find_out_more_images ?? []),
      JSON.stringify(example_images ?? []),
      format_min_en ?? null, format_min_ar ?? null,
      format_max_en ?? null, format_max_ar ?? null,
      thickness_min_en ?? null, thickness_min_ar ?? null,
      thickness_max_en ?? null, thickness_max_ar ?? null,
      materials_en ?? null, materials_ar ?? null,
      extras_en ?? null, extras_ar ?? null,
      enhancements_en ?? null, enhancements_ar ?? null,
    ],
  );
...
export async function update(req: Request, res: Response): Promise<void> {
  ...
  const { rows } = await pool.query(
    ...
    [
      category_id ?? null, slug, title_en, title_ar,
      subtitle_en ?? null, subtitle_ar ?? null,
      image_url, swatch_color ?? null, sort_order, is_active,
      description_en ?? null, description_ar ?? null,
      best_use_en ?? null, best_use_ar ?? null,
      eco_friendly_en ?? null, eco_friendly_ar ?? null, model_3d ?? null,
      JSON.stringify(find_out_more_images ?? []),
